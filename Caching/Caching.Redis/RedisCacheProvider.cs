    ...
    public class RedisCacheProvider : ICacheProvider, IDisposable
    {
        ...
        )
        {
            ...
            _redisMultiplexer = new Lazy<ConnectionMultiplexer>(
                () => ConnectionMultiplexer.Connect(_uri)
            );
            _database = new Lazy<IDatabaseAsync>(
                () => _redisMultiplexer.Value.GetDatabase(_dbNumber, _uri)
            );
            _logger = logger;
        ...
        public TryOptionAsync<T> Get<T>(Option<string> key)
        {
            return async () =>
            {
                ...
                var result = await ExecuteResiliently(
                    Database.StringGetAsync(redisKey),
                    redisKey.ToString()
                );
                return await Task.FromResult(
                    _jsonConverterProvider.Deserialize<T>(
                        result.HasValue ? result.ToString().Trim('"') : string.Empty
                    )
                );
            };
        }

        public TryOptionAsync<Unit> StoreInCache<T>(
            Option<string> key,
            T data,
            double expirationDurationInSeconds = 0
        )
        {
            return async () =>
            {
                ...
                var resilientExecutor = new Func<Task<bool>, Task<bool>>(work =>
                    ExecuteResiliently(work, redisKey.ToString())
                );
                        ...
                        if (expirationDurationInSeconds <= 0)
                            return resilientExecutor(
                                Database.StringSetAsync(
                                    redisKey,
                                    _jsonConverterProvider.Serialize(v)
                                )
                            );
                        else
                            return resilientExecutor(
                                Database.StringSetAsync(
                                    redisKey,
                                    _jsonConverterProvider.Serialize(v),
                                    TimeSpan.FromSeconds(expirationDurationInSeconds)
                                )
                            );
                    },
            ...
            var retryPolicy = Policy
                ...
                .WaitAndRetryAsync(
                    ...
                    (exception, _) =>
                    {
                        ...
                        _logger.LogWarning(
                            $"Timeout or cancellation exception. Retrying context: {context}"
                        );
                    }
                );
        ...
        private static RedisKey GetRedisKey(Option<string> key) =>
            key.Match(
                k => new RedisKey(k),
                () => throw new CacheException("Cache Key is empty", string.Empty)
            );
