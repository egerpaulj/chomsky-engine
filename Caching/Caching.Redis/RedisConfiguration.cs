...
namespace Caching.Redis
{
    public class RedisConfiguration : IRedisConfiguration
    {
        ...
        {
            RedisHostname =
                configuration[IRedisConfiguration.HostnameKey] ?? IRedisConfiguration.HostDefault;
            RedisPort = int.Parse(
                configuration[IRedisConfiguration.PortKey] ?? IRedisConfiguration.PortDefault
            );
            RedisDatabaseNumber = int.Parse(
                configuration[IRedisConfiguration.DatabaseIndexKey]
                    ?? IRedisConfiguration.DatabaseIndexDefault
            );
