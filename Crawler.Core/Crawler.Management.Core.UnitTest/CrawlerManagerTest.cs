...
namespace Crawler.Management.Core.UnitTest
{
    ...
    public class CrawlerManagerTest
    {
        ...
        public CrawlerManagerTest()
        {
            ...
            _cacheMock
                .Setup(c => c.UpdateCrawlCompleted(It.IsAny<Option<Guid>>()))
                .Returns(async () => await Task.FromResult(Unit.Default));

            _cacheMock
                .Setup(m => m.StoreCrawlEnded(It.IsAny<Option<Crawl>>()))
                .Returns(async () =>
                {
        ...
        public void CreateRequestFile()
        {
            ...
            var testFilePath = Path.Combine(
                "/home/user/vscode/csharp/crawler/TestData/Requests",
                "crawlRequest.json"
            );
        ...
        {
            var res = JsonConvert.DeserializeObject<CrawlRequest>(
                File.ReadAllText("Resources/56bc3065-fc3c-4af6-acc0-dda71f70c35f.json"),
        ...
        public void StartCrawlTest()
        {
            var t = _testee
                .Start()
                .Match(a => a, () => throw new Exception("Failed to start crawl"));

            // ACT
            File.Copy(
                "Resources/56bc3065-fc3c-4af6-acc0-dda71f70c35e.json",
                "Requests/in/56bc3065-fc3c-4af6-acc0-dda71f70c35e.json"
            );
