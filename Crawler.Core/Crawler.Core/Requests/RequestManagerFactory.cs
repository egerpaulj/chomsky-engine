...
namespace Crawler.Core.Requests
{
    ...
    public class RequestManagerFactory : IRequestManagerFactory
    {
        ...
        private Dictionary<string, IRequestManager> _requestManagers =
            new Dictionary<string, IRequestManager>();
        ...
        {
            var throttleUri = new Uri(
                uri.Match(
                    u => u,
                    () => throw new CrawlException("Throttle Uri is empty", ErrorType.ThrottleError)
                )
            );
            ...
            lock (_syncObject)
            {
                if (!_requestManagers.TryGetValue(throttleUri.Host, out var requestManager))
                {
                    var reqManager = new RequestManager(
                        _loggerFactory.CreateLogger<RequestManager>(),
                        _cache,
                        host
                    );
