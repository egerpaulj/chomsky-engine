...
namespace Crawler.Core.Parser.DocumentParts
{
    ...
    {
        public DocumentPartTableRow(Option<string> baseUri)
            : base(baseUri)
        {
            this.DocPartType = DocumentPartType.Row;

            this.Selector = new DocumentPartSelector { Xpath = ".//td" };
        }
                        ...
                        var cols = await columns.SelectAsync(async n =>
                        {
                            ...
                            await docpart
                                ...
                                .Match(
                                    u => { },
                                    () =>
                                        AppendAnomaly(AnomalyType.MissingRow, "Failed to parse row")
                                );
                            return docpart;
                        });
