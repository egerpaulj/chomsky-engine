...
namespace Crawler.Core.Parser.DocumentParts
{
    ...
    {
        public DocumentPartTable(Option<string> baseUri)
            : base(baseUri)
        {
            DocPartType = DocumentPartType.Table;
            Selector = new DocumentPartSelector { Xpath = "//table" };
        }
                    ...
                    return async () =>
                    {
                        return await MonitorPerformance.MonitorAsync(
                            async () =>
                            {
                                ...
                                Headers = (
                                    await tableElement
                                        ...
                                        .SelectAsync(async n =>
                                        {
                                            ...
                                            await docpart
                                                .Parse(CreateDocument(new List<HtmlNode> { n }))
                                                .Match(
                                                    u => { },
                                                    () =>
                                                        AppendAnomaly(
                                                            AnomalyType.MissingHeader,
                                ...
                                SubParts = (
                                    await tableElements
                                        .Where(t => t != tableElement)
                                        .SelectAsync(async n =>
                                        {
                                            var documentPartTable = new DocumentPartTable(BaseUri);

                                            await documentPartTable
                                                .Parse(CreateDocument(new List<HtmlNode> { n }))
                                                .Match(
                                                    u => { },
                                                    () =>
                                                        AppendAnomaly(
                                                            AnomalyType.MissingTable,
                                                            "Failed to parse Additionally matched table"
                                                        )
                                                );
        ...
        {
            var rows = Rows.Match(r => r, () => new List<DocumentPartTableRow>())
                .SelectMany(t => t.GetBriefSummary())
                .ConvertToString();
            var headers = Headers
                .Match(h => h, () => new List<DocumentPart>())
                .SelectMany(t => t.GetBriefSummary() + ". \n")
                .ConvertToString();
            return $"TABLE: \n Headers: \n{headers} \n Rows: \n{rows}";
        }
