...
namespace Crawler.Core.Parser.DocumentParts
{
    ...
    {
        public DocumentPartAutodetect(Option<string> baseUri)
            : base(baseUri)
        {
            DocPartType = DocumentPartType.AutoDetect;
            Selector = new DocumentPartSelector() { Xpath = ".//*[not(self::html)]" };
        }
                    ...
                    return async () =>
                    {
                        return await MonitorPerformance.MonitorAsync(
                            async () =>
                            {
                                ...
                                await documentPartArticle
                                    .Parse(document)
                                    .Match(
                                        u => { },
                                        () =>
                                            documentPartArticle.AppendAnomaly(
                                ...
                                if (tableNodes != null)
                                {
                                    var tableParts = tableNodes.Select(t =>
                                    {
                                        ...
                                        var res = documentPart
                                            .Parse(CreateDocument(new List<HtmlNode> { t }))
                                            .Match(
                                                u => { },
                                                () =>
                                                    documentPart.AppendAnomaly(
                                                        AnomalyType.MissingContent,
                                                        "Failed to parse content"
                                                    )
                                            )
        ...
        {
            var autoDetectText = SubParts
                .Match(t => t, () => new List<DocumentPart>())
