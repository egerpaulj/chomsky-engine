    ...
    public class DocumentPartSelector
    {
        ...
        public TryOptionAsync<IEnumerable<HtmlNode>> GetNodes(Option<HtmlDocument> document)
        {
            return async () =>
            {
                ...
                var allMatchedNodes = doc
                    .DocumentNode.SelectNodes(Xpath.Match(x => x, () => "//*[not(self::html)]"))
                    ?.ToList();

                var contentFilter = ContentSpecificMatch.Match(s => s, string.Empty);
                if (!string.IsNullOrEmpty(contentFilter))
                    allMatchedNodes = allMatchedNodes
                        .Where(node => node.InnerText.Contains(contentFilter))
                        .ToList();
                ...
                var filteredContentBasedNodes = allMatchedNodes
                    .Where(n => !allMatchedNodes.Any(n2 => n2.ChildNodes.Contains(n)))
                    .ToList();

                matchedNodes = matchedNodes.Append(filteredContentBasedNodes);

                return await Task.FromResult(
                    Option<IEnumerable<HtmlNode>>.Some(
                        matchedNodes.Distinct(HtmlNodeComparer.EqualityComparer)
                    )
                );
            };
        }
