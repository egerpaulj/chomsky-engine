...
namespace Crawler.Core.Parser.DocumentParts
{
    ...
    {
        public DocumentPartFile(Option<string> baseUri)
            : base(baseUri)
        {
            DocPartType = DocumentPartType.File;

            Selector = new DocumentPartSelector { Xpath = ".//*[self::a or self::img]" };
        }
                                ...
                                var doc = document.Match(
                                    ...
                                    () =>
                                        throw new CrawlException(
                                            "Can't prase an empty document for links",
            ...
            return nodes
                .Select(n =>
                {
                    if (n.Name.ToLower() == "img")
                    {
                        ...
                        {
                            part.AppendAnomaly(
                                AnomalyType.MissingImg,
                                $"missing Src in {n.InnerHtml}"
                            );
                        }
                    ...
                    if (uriHref.IsNone)
                    {
                        partLink.AppendAnomaly(
                            AnomalyType.MissingImg,
                            $"missing href in {n.InnerHtml}"
                        );
                    }
        ...
        public override string GetBriefSummary()
        {
            var downloadLinks = DownloadLinks
                .Match(h => h, () => new List<DocumentPartLink>())
                .SelectMany(t => t.GetBriefSummary() + "\n")
                .ConvertToString();
            var fileData = FileDataList
                .Match(h => h, () => new List<FileData>())
                .SelectMany(t => t.Name.Match(n => n, () => string.Empty) + "\n")
                .ConvertToString();
