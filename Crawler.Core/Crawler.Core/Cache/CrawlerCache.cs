    ...
    public class CrawlerCache : ICache
    {
        ...
        public TryOptionAsync<DateTime> GetLastRequestTime(Option<string> uri)
        {
            return uri.ToTryOptionAsync()
                .Bind(u => _cacheProvider.Get<long>(string.Format(LastRequestKey, u)))
                .Bind<long, DateTime>(ticks =>
                    async () => await Task.FromResult(new DateTime(ticks))
                );
        }
        ...
        public TryOptionAsync<Unit> StoreLastRequest(Option<string> uri)
        {
            return uri.ToTryOptionAsync()
                .Bind(u =>
                    _cacheProvider.StoreInCache(
                        string.Format(LastRequestKey, u),
                        DateTime.UtcNow.Ticks,
                        LastRequestExpiryInSeconds
                    )
                );
        }
        ...
        public TryOptionAsync<Unit> StoreCrawlEnded(Option<Crawl> crawl)
        {
            return crawl
                ...
                .Bind(id =>
                    _cacheProvider.StoreInCache(
                        string.Format(CrawlKey, id.ToString()),
                        $"Crawl InProgress: {GetDateTime}"
                    )
                );
        }
        ...
        {
            return uri.ToTryOptionAsync()
                .Bind(u => _cacheProvider.Get<bool>(string.Format(ActiveDownloadKey, u)));
        }
        ...
        public TryOptionAsync<Unit> SetActiveDownload(Option<string> uri, bool downloadState)
        {
            return uri.ToTryOptionAsync()
                .Bind(u =>
                    _cacheProvider.StoreInCache(string.Format(ActiveDownloadKey, u), downloadState)
                );
        }

        private static string GetDateTime =>
            DateTime.UtcNow.ToString(ICacheProvider.DateTimeFormat);
    }
}
