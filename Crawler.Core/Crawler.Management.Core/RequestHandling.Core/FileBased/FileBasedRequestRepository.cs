...
{
    public class FileBasedRequestRepository
        : IRequestProvider,
            IResponsePublisher,
            IFailurePublisher,
            IRequestPublisher
    {
        ...
        public FileBasedRequestRepository(IJsonConverterProvider jsonConverterProvider)
            : this(
                new DirectoryInfo(Path.Combine(Environment.CurrentDirectory, "RequestRepository")),
                5000,
                jsonConverterProvider
            ) { }

        public FileBasedRequestRepository(
            Option<DirectoryInfo> workingDirectory,
            int pollIntervalInMs,
            IJsonConverterProvider jsonConverterProvider
        )
        {
            _workingDirectory = workingDirectory.Match(
                d => d,
            ...
            var res = Init()
                .Match(
                    r => r,
                    () => throw new Exception("Failed to initialize folders in File repository")
                )
                .Result;
        ...
        public IObservable<Either<CrawlRequest, CrawlRequestException>> GetObservable(
            ...
            Func<CrawlRequest, Task<LanguageExt.Unit>> crawlTask
        )
        {
            return Observable
                ...
                .Select(args =>
                {
                    ...
                    try
                    {
                        var request = JsonConvert.DeserializeObject<CrawlRequest>(
                            File.ReadAllText(args.FullPath, IJsonConverterProvider.TextEncoding),
                            _jsonConverterProvider.GetJsonConverters()
                        );
                        File.Move(
                            args.FullPath,
                            Path.Combine(_workingDirectory.FullName, args.Name),
                            overwrite: true
                        );
                        either = request;
                        return either;
                    ...
                    catch (Exception e)
                    {
                        ...
                        File.Move(
                            args.FullPath,
                            Path.Combine(_errorDirectory.FullName, $"args.Name_error"),
                            overwrite: true
                        );
                ...
                .SelectMany(eitherRes =>
                    Observable.FromAsync<Either<CrawlRequest, CrawlRequestException>>(async () =>
                    {
                        return await eitherRes.MatchAsync(
                            ex => eitherRes,
                            async req =>
                            {
        ...
        public TryOptionAsync<Unit> PublishFailure(Option<CrawlRequest> request)
        {
            return Init()
                .Bind<Unit, Unit>(u =>
                    async () =>
                    {
                        var crawlrequest = request.Match(
                            r => r,
                            () =>
                                throw new CrawlException(
                                    "Unable to publish an empty request",
                                    ErrorType.PublishError
                                )
                        );
                        try
                        {
                            ...
                            File.WriteAllText(
                                Path.Combine(
                                    _errorDirectory.FullName,
                                    crawlrequest.CrawlId.Match(id => id.ToString(), () => "unknown")
                                ),
                                json,
        ...
        public TryOptionAsync<Unit> PublishResponse(Option<CrawlResponse> response)
        {
            return Init()
                .Bind<Unit, Unit>(u =>
                    async () =>
                    {
                        var crawlresponse = response.Match(
                            r => r,
                            () => throw new Exception("Unable to publish an empty response")
                        );
                        ...
                        try
                        {
                            ...
                            File.WriteAllText(
                                Path.Combine(
                                    _outDirectory.FullName,
                                    crawlresponse.CrawlerId.Match(
                                        id => id.ToString(),
                                        () => "unknown"
                                    )
                                ),
                            ...
                            var requestMatch = _workingDirectory
                                .GetFiles()
                                .FirstOrDefault(f =>
                                    f.Name.Contains(
                                        crawlresponse.CrawlerId.Match(
                            ...
                            System.Console.WriteLine(
                                $"{DateTime.Now:HH:mm:ss.fff}: Finished Processing request"
                            );
                            System.Console.WriteLine(
                                $"{DateTime.Now:HH:mm:ss.fff}: RESPONSE: \n {document.GetBriefSummary()}"
                            );
            ...
            return Init()
                .Bind<Unit, Unit>(u =>
                    async () =>
                    {
                        var ls = links.Match(
                            r => r,
                            () => throw new Exception("Unable to publish an empty links")
                        );
                        if (links.Count() > 0)
                        {
                            foreach (var l in ls)
                            {
                                ...
                                File.WriteAllText(
                                    Path.Combine(_outDirectory.FullName, Guid.NewGuid().ToString()),
                                    json,
                                    IJsonConverterProvider.TextEncoding
                                );
                            }
        ...
        private bool AllDirectoriesExists() =>
            _workingDirectory.Exists
            && _inDirectory.Exists
            && _outDirectory.Exists
            && _errorDirectory.Exists;
    }
}
