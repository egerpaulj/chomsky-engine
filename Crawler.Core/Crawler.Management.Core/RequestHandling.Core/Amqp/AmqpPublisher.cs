...
namespace Crawler.Management.Core.RequestHandling.Core.Amqp
{
    public class AmqpPublisher : IResponsePublisher, IFailurePublisher
    {
        ...
        {
            _failurePublisher = amqpProvider
                .GetPublisher(FailurePublisherContext)
                .Match(
                    p => p,
                    () => throw new System.Exception("Failed to get AMQP failure publisher"),
                    ex =>
                    {
                        throw ex;
                    }
                ...
                .Result;
            _responsePublisher = amqpProvider
                .GetPublisher(ResponsePublisherContext)
                .Match(
                    p => p,
                    () => throw new System.Exception("Failed to get AMQP response publisher"),
                    ex =>
                    {
                        throw ex;
                    }
