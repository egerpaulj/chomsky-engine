    ...
    public class RabbitMqIntegrationTest
    {
        ...
        public async Task SetUp()
        {
            ...
            _configuration = TestHelper.TestHelper.GetConfiguration(
                $"{TestHelper.TestHelper.GetEnvironment()}.Rabbitmq"
            );

            _amqpBootstrapper = new AmqpBootstrapper(_configuration);
            await _amqpBootstrapper
                .Bootstrap()
                .Match(r => { }, () => throw new Exception("Failed to bootstrap amqp"));
        }
        ...
        public async Task CreateExchangeFromConfiguration_WhenFileCopied_ThenTransformedToOutput()
        {
            ...
            File.Move(
                $"testData/rabbitmq/{fileNameAsGuid}",
                $"testData/rabbitmq/in/{fileNameAsGuid}"
            );
                ...
                .ConfigureServices(
                    (hostContext, services) =>
                    {
                        ...
                        services.AddTransient<
                            IMessageHandler<TestOutputMessage, TestOutputMessage>,
                            MessageHandler
                        >();
                    }
                )
