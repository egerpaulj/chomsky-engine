...
using Moq;
using BertrandStateDataModel = Microservice.Exchange.Bertrand.BertrandStateDataModel;
...
public class BertrandExchangeIntegrationTest
{
    ...
    private readonly ElasticsearchBertrandConsumer<TestOutputMessage> elasticsearchBertrandConsumer;
    ...
    {
        var configuration =
            TestHelper.TestHelper.GetConfiguration(
                $"{TestHelper.TestHelper.GetEnvironment()}.Bertrand"
            ) as IConfiguration;
        ...
        var messageublisher = _amqpProvider
            .GetPublisher(RabbitMqOutputContext)
            .Match(r => r, () => throw new System.Exception("Failed"))
            .Result;

        var bertrandPublisherRabbitMq = new RabbitMqPublisher<object>("Rabbit", messageublisher);

        rabbitMqConsumer = new RabbitMqBertrandConsumer<TestOutputMessage>(
            _amqpProvider,
            RabbitMqInputContext,
        ...
        // -- MongoDB
        var databaseConfiguration = new DatabaseConfiguration("bertrand_exchange_test", "test");
        var pollingFactory = new PollingConsumerFactory(
            loggerFactory: loggerFactory,
            "bertrand_input"
        );
        var repositoryFactory = new Microservice.Mongodb.Repo.RepositoryFactory(
            configuration,
        ...
        // -- ES
        var elasticRepoFactory = new Microservice.Elasticsearch.Repo.RepositoryFactory(
            loggerFactory,
            configuration,
            jsonConverterProvider
        );
        _elasticSearchRepository = elasticRepoFactory.CreateRepository();
        elasticsearchBertrandConsumer = new ElasticsearchBertrandConsumer<TestOutputMessage>(
            "es-integration-test",
            loggerFactory,
            "{\"query\": {\"match\": {\"EnrichedData\": {\"query\": \"some test data\"}}}}",
            EsIndexName,
            elasticRepoFactory,
            pollingFactory,
            200
        ...
        // -- Transformers
        var routingKeyTransformer = new TestTransformer(
            "bertrand_output_mongo",
            false,
            "Test transformer"
        );
        var routingKeyTransformerEs = new TestTransformer(
            "bertrand_output_es",
            true,
            "ES Test Transformer"
        );
        ...
        var bertrandPublisherElasticsearch = new BertrandPublisher<TestEsOutputMessage>(
            "Elastic",
            new ElasticsearchPublisher<TestEsOutputMessage>(
                "Elastic",
                EsIndexName,
        ...
        [
            ...
            new BertrandRoutingKeyFilter("bertrand_output_mongo", "MongoDb"),
            new BertrandRoutingKeyFilter("bertrand_output_es", "Rabbit"),
    ...
    public async Task FromMongodbTo()
    {
        ...
        await _mongodbRepository
            .Delete(FilterDefinition<BsonDocument>.Empty)
            .Match(r => r, () => throw new System.Exception("Faile to purge collection"));
        await _bertrandStateDeadletterRepository
            .Delete(FilterDefinition<BsonDocument>.Empty)
            .Match(r => r, () => throw new System.Exception("Faile to purge collection"));
        await _bertrandStateRepository
            .Delete(FilterDefinition<BsonDocument>.Empty)
            .Match(r => r, () => throw new System.Exception("Faile to purge collection"));
        await _bertrandExchangeRepository
            .Delete(FilterDefinition<BsonDocument>.Empty)
            .Match(r => r, () => throw new System.Exception("Faile to purge collection"));
        ...
        await _mongodbRepository
            .AddOrUpdate(new TestOutputMessage { OriginalData = "I am some data" })
            .Match(r => r, () => throw new System.Exception("Failed to update DB"));
        ...
        await exchange
            .Start()
            .Match(m => m, () => throw new System.Exception("Failed to start exchange"));
        ...
        var mongodbResultList = await _mongodbRepository
            .GetMany(
                Builders<BsonDocument>.Filter.Regex(
                    "EnrichedData",
                    BsonRegularExpression.Create("Transformed data: bertrand_output")
                )
            )
            .Match(r => r, () => throw new Exception("Failed to get Mongodb"));
    ...
    public async Task FromRabbitMqToMongoDb()
    {
        ...
        await _amqpProvider
            ...
            .Bind(publisher =>
                publisher.Publish<TestOutputMessage>(
                    new TestOutputMessage { OriginalData = "I am some data" }
                )
            )
    ...
    public async Task FromRabbitMq_ToBadTransformer_ToDeadletter()
    {
        ...
        var deadletter = await _bertrandStateDeadletterRepository
            .GetMany(Builders<BsonDocument>.Filter.Empty)
            .Match(r => r, () => throw new Exception("Failed to get deadletter"), ex => throw ex);
        Assert.AreEqual(1, deadletter.Count);

        // ASSERT - cleared the state (i.e. moved to deadletter)
        var outstanding = await bertrandStateStore
            .GetOutstandingMessages()
            .Match(r => r, () => throw new Exception("Nothing in the deadletter"), ex => throw ex);
    ...
    public async Task FromState_ToPublishers()
    {
        ...
        await _bertrandStateRepository
            .AddOrUpdate(
                new BertrandStateDataModel
                {
                    CorrelationId = Guid.NewGuid(),
                    Id = Guid.NewGuid(),
                    RoutingKey = "bertrand_input",
                    Payload = new EmptyJsonConverterProvider().Serialize(
                        new TestOutputMessage { OriginalData = "I am some data" }
                    ),
...
public class TestTransformer(string routingKey, bool isEs, string name) : IBertrandTransformer
{
    ...
    private TryOptionAsync<Message<object>> Transform(Message<object> input)
    {
        return async () =>
        {
            ...
            if (isEs)
            {
                var inputPayload = (TestOutputMessage)
                    input.Payload.Match(
                        p => p,
                        () => throw new System.Exception("Payload is empty")
                    );
                ...
                output.Payload = new TestEsOutputMessage
                {
                    ...
                    EnrichedData =
                        "Transformed data: "
                        + routingKey
                        + DateTime.Now.ToString(" yyyy.MM.dd: mm.ss.fff"),
                    Id = Guid.NewGuid(),
                };
            ...
            var payload = (TestOutputMessage)
                input.Payload.Match(p => p, () => throw new System.Exception("Payload is empty"));
            payload.EnrichedData =
                "Transformed data: " + routingKey + DateTime.Now.ToString(" yyyy.MM.dd: mm.ss.fff");
