    ...
    public IObservable<Either<Message<CsvData>, ConsumerException>> GetObservable()
    {
        return _observable.Select(result =>
        {
            return result
                ...
                .MapLeft(data => new Message<CsvData>() { Id = Guid.NewGuid(), Payload = data });
        });
    }
...
#pragma warning disable CS8602 // null
        return cancelPreviousRun.Bind(_ =>
            _csvFileReader.Read(_filePath, _observer, _cancellationTokenSource.Token)
        );
