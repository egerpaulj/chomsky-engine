...
{
    TryOptionAsync<Unit> Read(
        Option<string> filePath,
        IObserver<Either<CsvData, Exception>> observer,
...
public class CsvFileReader : ICsvFileReader
{
    ...
    public TryOptionAsync<Unit> Read(
        Option<string> filePath,
        IObserver<Either<CsvData, Exception>> observer,
        CancellationToken cancelToken
    )
                            ...
                            try
                            {
                                ...
                                if (data == null || headers == null)
                                    throw new InvalidDataException(
                                        "CSV Headers and Data should exist"
                                    );

                                var csvData = await CsvDataFactory
                                    .CreateCsv(headers, data)
                                    .Match(d => d, () => throw new Exception("Failed to "));
                                observer.OnNext(csvData);

                                _logger.LogInformation(
                                    $"Processed line: {lineNumber}, file: {file}"
                                );
                            }
                            catch (Exception ex)
                            {
                                _logger.LogError(
                                    ex,
                                    $"Failed to process line: {lineNumber}, file: {file}"
                                );
                                observer.OnNext(ex);
                            }
