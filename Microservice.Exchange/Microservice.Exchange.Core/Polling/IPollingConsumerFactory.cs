...
public class PollingConsumerFactory(ILoggerFactory loggerFactory, string routingKey = "") : IPollingConsumerFactory
{
    readonly ILoggerFactory loggerFactory = loggerFactory;
    readonly string routingKey = routingKey;

    public IPollingConsumer<T> Create<T>(Func<TryOptionAsync<List<T>>> queryDataFunc, int intervalInMs)
    {
        return new PollingConsumer<T>(loggerFactory.CreateLogger<IConsumer<T>>(), queryDataFunc, intervalInMs, routingKey);
    }
}