    ...
    public class MessageExchange<T, R> : IMessageExchange<T, R>
    {
        ...
        public TryOptionAsync<Unit> End()
        {
            return async () =>
            {
                _logger.LogInformation(
                    $"#### Microservice Exchange: {_exchangeName}: Stopping Exchange"
                );
                Dispose();
        ...
        public TryOptionAsync<Unit> Start()
        {
            return async () =>
            {
                ...
                _logger.LogInformation(
                    $"#### Microservice Exchange: {_exchangeName}: Starting Exchange"
                );

                if (
                    Directory.Exists(_workDirectoryPath)
                    && Directory.GetFiles(_workDirectoryPath).Any()
                )
                {
                    _logger.LogWarning(
                        $"#### Microservice Exchange: {_exchangeName}: Processing old unfinished/crashed work"
                    );
                    foreach (var file in Directory.GetFiles(_workDirectoryPath))
                    {
                        await ProcessMessage(
                            _jsonConverterProvider.Deserialize<Message<T>>(
                ...
                _logger.LogInformation(
                    $"#### Microservice Exchange: {_exchangeName}: Exchange started successfully"
                );
        ...
        private async Task<Unit> ProcessMessage(Message<T> dataIn)
        {
            ...
            {
                _logger.LogInformation(
                    $"#### Microservice Exchange: {_exchangeName}: Received Empty Message: Nothing to do"
                );
                return Unit.Default;
            }
            _logger.LogInformation(
                $"#### Microservice Exchange: {_exchangeName}: Received Message: {dataIn.Id}. CorrelationId: {dataIn.CorrelationId}"
            );
            _exchangeMetrics.IncInput(typeof(T).Name);

            var matchedPublishers = FilterPublishers(dataIn);
            _logger.LogInformation(
                $"#### Microservice Exchange: {_exchangeName}: Message: {dataIn.Id}. Number of matched publishers: {matchedPublishers.Count()}"
            );

            try
            {
                await File.WriteAllTextAsync(
                    GetWorkingFilePath(dataIn.Id),
                    _jsonConverterProvider.Serialize(dataIn),
                    IJsonConverterProvider.TextEncoding
                );

                var dataOut = await Transform(dataIn);
                _logger.LogInformation(
                    $"#### Microservice Exchange: {_exchangeName}: Message: {dataIn.Id}. Data transformed successfully"
                );
                ...
                await Parallel
                    .ForEach(
                        ...
                        {
                            _logger.LogInformation(
                                $"#### Microservice Exchange: {_exchangeName}: Message: {dataIn.Id}. Publishing to: {publisher.Name}"
                            );
                                ...
                                .Match(
                                    ...
                                    async () =>
                                        await ProcessError(
                                            dataOut,
                                            new Exception(
                                                $"Failed to publish. Publisher: {publisher.GetType().Name}"
                                            ),
                                            publisher.Name
                                        ),
                                    async ex =>
                                        await ProcessError(
                                            dataOut,
                                            new Exception(
                                                $"Failed to publish. Publisher: {publisher.GetType().Name}. {ex.Message} - {ex.StackTrace}"
                                            ),
                                            publisher.Name
                                        )
                                );
        ...
        {
            return Path.Combine(
                $"{_workDirectoryPath}{id.Match(i => i, () => Guid.NewGuid()).ToString()}"
            );
        }
        ...
        private async Task<Unit> ProcessError(Exception exception)
        {
            _logger.LogError(
                exception,
                $"#### Microservice Exchange: {_exchangeName}: Error received from consumer"
            );

            return await _deadletterPublisher
                ...
                .Match(
                    ...
                    () =>
                    {
                        _logger.LogError(
                            exception,
                            $"#### Microservice Exchange: {_exchangeName}: Deadletter publish failed"
                        );
                        StoreFailure(
        ...
        private async Task<Unit> ProcessError(Message<T> message, Exception ex)
        {
            _logger.LogError(
                ex,
                $"#### Microservice Exchange: {_exchangeName}: Message: {message.Id}. Error Processing Message"
            );

            return await _deadletterPublisher
                .PublishError(new ErrorMessage<T>(message, ex))
                .Match(
                    r => { },
                    () =>
                    {
                        _logger.LogError(
                            ex,
                            $"#### Microservice Exchange: {_exchangeName}: Deadletter publish failed. Id: {message.Id}"
                        );
                        StoreFailure(
                            _jsonConverterProvider.Serialize(message),
                            message.Id.ToString()
                        );
                    }
                );
        ...
        private async Task<Unit> ProcessError(
            Message<R> message,
            Exception ex,
        ...
        )
        {
            _logger.LogError(
                ex,
                $"#### Microservice Exchange: {_exchangeName}: Message: {message.Id}. Error Publishing"
            );

            _exchangeMetrics.IncError(publisherName);

            return await _deadletterPublisher
                .PublishError(new ErrorMessage<R>(message, ex))
                .Match(
                    r => { },
                    () =>
                    {
        ...
        private List<IPublisher<R>> FilterPublishers(Message<T> dataIn)
        {
            return _publishers
                .Where(p =>
                    _filter
                        .IsMatch(dataIn, Option<IPublisher<R>>.Some(p))
                        .Match(
                            r => r,
                            () =>
                            {
                                _logger.LogWarning(
                                    $"#### Microservice Exchange: {_exchangeName}: Filter error. Publishing to all configured publishers"
                                );
                                return true;
                            }
                        )
                ...
                .Match(
                    ...
                    () =>
                        throw new TransformationException(
                            $"Failed to transform data: Empty result. Transformer: {_transformer.GetType().Name}"
                        ),
                    ex =>
                        throw new TransformationException(
                            $"Failed to transform data. Transformer: {_transformer.GetType().Name}. Message: {ex.Message}",
                            ex
                        )
                );
            ;
        ...
        {
            var criticalFailureDirectory = new DirectoryInfo(
                Path.Combine(Environment.CurrentDirectory, ".exchangeFailures")
            );
            CreateIfNotExist(criticalFailureDirectory);
            File.WriteAllText(
                Path.Combine(criticalFailureDirectory.FullName, fileName),
                json,
                IJsonConverterProvider.TextEncoding
            );
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!isDisposed)
            {
                ...
                {
                    _logger.LogInformation(
                        $"#### Microservice Exchange: {_exchangeName}: Disposing Exchange"
                    );

                    _subscriptions.ForEach(sub => sub.Dispose());

                    _consumers.ForEach(async cons =>
                        await cons.End()
                            .Match(
                                r => { },
                                () =>
                                    _logger.LogWarning(
                                        $"#### Microservice Exchange: {_exchangeName}: Failed to stop consumer: {cons.GetType().Name}"
                                    ),
                                ex =>
                                    _logger.LogWarning(
                                        ex,
