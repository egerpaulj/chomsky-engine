    ...
    public class ConsolePublisher<T, R> : IPublisher<R>, IDeadletterPublisher<T, R>
    {
        ...
        public TryOptionAsync<Unit> Publish(Option<Message<R>> message)
        {
            return message
                ...
                {
                    Console.WriteLine(
                        $"## Console publisher: Publishing Message. Id: {message.Id}"
                    );
                    Console.WriteLine(
                        $"## Console publisher: Publishing Message. CorrelationId: {message.CorrelationId}"
                    );
        ...
        public TryOptionAsync<Unit> PublishError(Option<ErrorMessage<T>> message)
        {
            return message
                ...
                {
                    Console.WriteLine(
                        $"## Console publisher: Publishing Error Message. Id: {message.Message.Id}"
                    );
                    Console.WriteLine(
                        $"## Console publisher: Publishing Error Message. CorrelationId: {message.Message.CorrelationId}"
                    );
                    Console.WriteLine(_jsonConverterProvider.Serialize(message));
