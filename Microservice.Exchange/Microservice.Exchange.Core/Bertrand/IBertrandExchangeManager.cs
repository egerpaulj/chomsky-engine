...
public class BertrandExchangeManager(
    IBertrandExchangeStore bertrandExchangeStore,
    ILogger<BertrandExchangeManager> logger
) : IBertrandExchangeManager
{
    ...
    public TryOptionAsync<Unit> RegisterExchange(IBertrandExchange bertrandExchange)
    {
        return async () =>
        {
            var model = await bertrandExchangeStore
                .GetExchange(bertrandExchange.ExchangeName)
                .Match(
                    m => m.BertrandExchangeModel,
                    () =>
                        new BertrandExchangeModel()
                        {
                            ExchangeName = bertrandExchange.ExchangeName,
                        },
                    ex => throw ex
                );
            ...
            foreach (var transformer in bertrandExchange.GetTransformers())
            {
                ...
                {
                    model.Transformers.Add(
                        new BertrandExchangeControlModel
                        {
        ...
        return async () =>
        {
            ...
            timer.Elapsed += async (_, _) =>
            {
                ...
                foreach (var exchange in registeredExchanges)
                {
                    ...
                    {
                        var shouldStopConsumer = await bertrandExchangeStore
                            .IsConsumerActive(exchange.ExchangeName, consumer.Name)
                            .Match(r => r, false);
                        if (shouldStopConsumer)
                            await consumer
                                ...
                                .Match(
                                    ...
                                    () =>
                                        logger.LogWarning(
                                            "{ExchangeName} exchange: failed to stop consumer: {consumer}",
