pipeline {
  ...
  stages {
    ...
    stage('Data Service') {
      steps {
        sh 'dotnet publish -c Release Crawler.Data/Crawler.Data.Service/Crawler.Data.Service.csproj'
      }
    }
    ...
    stage('Containerize') {
      steps {
        ...
        sh 'docker build --tag registry:5000/crawler/data_service:${BRANCH_NAME}_$BUILD_ID Crawler.Data/Crawler.Data.Service/.'
      }
    }
    ...
    stage('Start Production Crawler') {
      steps {
        ...
        sh 'docker run -d --rm --network development_network -e ASPNETCORE_ENVIRONMENT="Production" --name data_service registry:5000/crawler/data_service:${BRANCH_NAME}_$BUILD_ID'
      }
    }
  }
  environment {
