    ...
    public abstract class RpcServiceBase<T, R> : Rpc.RpcBase
    {
        ...
        public override async Task<RpcReply> Execute(RpcRequest request, ServerCallContext context)
        {
            _logger.LogInformation(
                $"Received Grpc request on {_name}, CorrelationId: {request.Correlationid}"
            );
            ...
            try
            {
                ...
                var result = await Execute(genericRequest)
                    .Match(
                        x => x,
                        () => throw new Exception("Grpc Service Error - Nothing returned"),
                        ex => throw ex
                    );
                ...
                _logger.LogInformation(
                    $"Successfully processed Grpc request on {_name}, CorrelationId: {request.Correlationid}"
                );
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    $"Grpc Error on {_name}, CorrelationId: {request.Correlationid}"
                );
