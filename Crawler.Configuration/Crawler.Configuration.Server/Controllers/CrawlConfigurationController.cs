...
namespace Crawler.Configuration.Server.Controllers
{
    ...
    public class CrawlConfigurationController : ControllerBase
    {
        ...
        public CrawlConfigurationController(ILogger<CrawlConfigurationController> logger, ICrawlerConfigurationService config)
        {
            _logger = logger;
        ...
        public async Task<ActionResult<CrawlRequest>> GetCrawlRequest([FromBody]ConfigurationCrawlRequest request)
        {
            return await GetRequest(request, out var uri)
            .Match(r => r,
            () => throw new Exception($"Result was empty for CrawlConfiguration CrawlRequest for: {uri.AbsoluteUri}"),
            e => throw new Exception($"Error obtaining configuration for: {uri.AbsoluteUri}", e));
        }

        [Route("getcollectorcrawlrequest")]
        public async Task<ActionResult<CrawlRequestModel>> GetCollectorCrawlRequest([FromBody]SingleValue uri)
        {
            return await _configurationService.GetCollectorCrawlRequest(uri.Value)
            .Match(r => r,
            () => throw new Exception($"Result was empty for crawl request model. URI: {uri.Value}"),
            e => throw new Exception($"Error getting crawl request model. URI: {uri.Value}", e));
        }
        ...
        {
            var uri = request.Uri.Match(u => new Uri(u), () => throw new Exception("Request Uri is empty"));
            var crawlId = request.CrawlId.Match(g => g, () => Guid.Empty);
            
            return await _configurationService.GetUiActions(uri.AbsoluteUri, HttpContext.GetCorrelationId(), crawlId).Match(r => r, 
            () => new List<UiAction>(), 
            e => throw new Exception($"Error obtaining UiActions for: {uri}", e));
        }

        [Route("getdocumentpart")]
        public async Task<DocumentPart> GetDocumentPart([FromBody] ConfigurationCrawlRequest request)
        {
            var uri = request.Uri.Match(u => new Uri(u), () => throw new Exception("Request Uri is empty"));
            var crawlId = request.CrawlId.Match(g => g, () => Guid.Empty);
        ...
        {
            return await _configurationService.GetUnscheduledCrawlUriData().Match(r => r, 
        ...
        {
            return await _configurationService.GetPeriodicUri().Match(r => r, 
        ...
        {
            return await _configurationService.GetUriData(Guid.Parse(id.Value)).Match(r => r, 
            () => throw new Exception($"Empty result obtaining Uri Data"),
            e => throw new Exception($"Error obtaining Uri Data", e));
        }

        private TryOptionAsync<CrawlRequest> GetRequest(ConfigurationCrawlRequest request, out Uri uri)
        {
            uri = request.Uri.Match(u => new Uri(u), () => throw new Exception("Request Uri is empty"));
            var crawlId = request.CrawlId.Match(g => g, () => Guid.Empty);

            return _configurationService.CreateRequest(uri.AbsoluteUri, HttpContext.GetCorrelationId(), crawlId);
        }
