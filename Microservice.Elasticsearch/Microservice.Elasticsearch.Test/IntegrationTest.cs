...
namespace Microservice.Elasticsearch.Test
{
    ...
    public class IntegrationTest
    {
        ...
        public async Task Setup()
        {
            ...
            _testee = new ElasticsearchRepository(
                Mock.Of<ILogger<ElasticsearchRepository>>(),
                configuration,
                Mock.Of<IJsonConverterProvider>()
            );

            var node = new Uri(
                configuration.GetConnectionString(ElasticsearchRepository.ConnectionStringKey)
            );
        ...
        public async Task IndexObject_ThenObjectIndexed()
        {
            // ARRANGE
            var id = Guid.NewGuid();
            var testData = new TestDataModel() { Id = id, Data = $"Some test information: {id}" };
            ...
            var resultList = await _testee
                .Search<TestDataModel>(TestIndexKey, query)
                .Match(r => r, () => throw new Exception("Empty"), ex => throw ex);
