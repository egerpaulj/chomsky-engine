...
namespace Microservice.Core.Http
{
    public class HttpClientService : IHttpClientService
    {
        ...
        public HttpClientService(
            ...
            ILogger<HttpClientService> httpClientService,
        ...
        public TryOptionAsync<T> Get<T>(Option<Guid> correlationId, Option<string> uri)
        {
            return GetParameters(correlationId, uri)
                .Bind<Tuple<Guid, string>, T>(tuple =>
                    async () =>
                    {
                        HttpRequestMessage httpRequestMessage = CreateRequest(
                            tuple.Item1,
                            tuple.Item2,
                            HttpMethod.Get
                        );
        ...
        public TryOptionAsync<string> GetStringContent(
            Option<Guid> correlationId,
            Option<string> uri
        )
        {
            return GetParameters(correlationId, uri)
                .Bind<Tuple<Guid, string>, string>(tuple =>
                    async () =>
                    {
                        HttpRequestMessage httpRequestMessage = CreateRequest(
                            tuple.Item1,
        ...
        public TryOptionAsync<T> Send<R, T>(
            Option<Guid> correlationId,
            Option<R> send,
            Option<string> uri,
                ...
                .Bind<Tuple<Guid, string>, T>(tuple =>
                    async () =>
                    {
                        HttpRequestMessage httpRequestMessage = CreateRequest(
                            tuple.Item1,
                            tuple.Item2,
                            method.Match(m => m, () => HttpMethod.Get)
                        );

                        send.Match(
                            s =>
                            {
                                var content = typeof(R).IsValueType
                                    ? s.ToString()
                                    : _converterProvider.Serialize(s);
        ...
        public TryOptionAsync<Unit> Send<R>(
            Option<Guid> correlationId,
            Option<R> send,
            Option<string> uri,
            Option<HttpMethod> method
        ...
        {
            return GetParameters(correlationId, uri)
                .Bind<Tuple<Guid, string>, Unit>(tuple =>
                    async () =>
                    {
                        HttpRequestMessage httpRequestMessage = CreateRequest(
                            tuple.Item1,
        ...
        private static TryOptionAsync<Tuple<Guid, string>> GetParameters(
            Option<Guid> correlationId,
            Option<string> uri
        )
        {
            return Option<Guid>
                .Some(correlationId.Match(c => c, () => Guid.NewGuid()))
                .ToTryOptionAsync()
                .Bind<Guid, Tuple<Guid, string>>(guid =>
                    uri.ToTryOptionAsync()
                        .Bind<string, Tuple<Guid, string>>(s =>
                            async () => await Task.FromResult(Tuple.Create<Guid, string>(guid, s))
                        )
                );
        }
        ...
        )
        {
            ...
            httpRequestMessage.Headers.Add(
                CorrelationIdMiddlware.CorrIdHeaderKey,
            ...
            var retryPolicy = Policy
                ...
                .WaitAndRetryAsync(
                    ...
                    (exception, _) =>
                    {
                        ...
                        _logger.LogWarning(
                            $"Timeout or cancellation exception. Retrying request: {request.RequestUri}"
                        );
                    }
                );

            var response = await retryPolicy.ExecuteAsync<HttpResponseMessage>(
