...
namespace Crawler.WebDriver.Grpc.Client
{
    public class GrpcWebDriverService : IWebDriverService
    {
        ...
        public GrpcWebDriverService(
            IConfiguration configuration,
            ILogger<GrpcWebDriverService> logger,
            IGrpcMetrics metrics,
            IJsonConverterProvider jsonConverterProvider
        )
        {
            _logger = logger;
            _rpcClient = new RpcClient<DriverRequest, DriverResponse>(
                logger,
                metrics,
        ...
        public TryOptionAsync<FileData> Download(Option<DownloadRequest> request)
        {
            ...
            var uri = request
                ...
                .Match(
                    u => u,
                    () => throw new DownloadException($"Failed to download. Uri is empty")
                );

            _logger.LogInformation(
                $"Sending Grpc Request. Download data: {uri}, CorrelationId: {correlationId}"
            );
            return _rpcClient
                .Execute(
                    new DriverRequest() { DownloadRequest = request },
                    _grpcServiceUri,
                    correlationId
                )
                .Bind<DriverResponse, FileData>(response =>
                    async () =>
                        await Task.FromResult(
                            response.DownloadRequest.Match(
                                d => d,
                                () =>
                                    throw new DownloadException(
                                        $"Failed to download data: {uri}. Data is empty"
                                    )
                            )
                        )
                );
        ...
        public TryOptionAsync<string> LoadPage(Option<LoadPageRequest> request)
        {
            ...
            var uri = request
                .Bind(r => r.Uri)
                .Match(u => u, () => throw new PageLoadException("", "Uri is empty"));
            _logger.LogInformation(
                $"Sending Grpc Request. Load Page: {uri}, CorrelationId: {correlationId}"
            );
            return _rpcClient
                .Execute(
                    new DriverRequest() { LoadPageRequest = request },
                    _grpcServiceUri,
                    correlationId
                )
                .Bind<DriverResponse, string>(response =>
                    async () =>
                        await Task.FromResult(
                            response.LoadPageRequest.Match(
                                d => d,
                                () =>
                                    throw new PageLoadException(
                                        uri,
                                        "Failed to load page. XDocument is empty"
                                    )
                            )
                        )
                );
