...
namespace Crawler.WebDriver.Selenium.Firefox
{
    public class WebDriverFirefoxRobust : IWebDriverService
    {
        ...
        private const int CLEANUP_MINUTES = 30;
        ...
        public WebDriverFirefoxRobust(IWebDriverMetrics metrics, ILogger<WebDriverServiceFirefox> logger)
        : this(metrics, logger, true)
        {
        }

        public WebDriverFirefoxRobust(IWebDriverMetrics metrics, ILogger<WebDriverServiceFirefox> logger, bool isHeadless)
        {
            _logger = logger;
            ...
            _cleanupTimer = new Timer(x => CleanUpWebDrivers("*"), this, TimeSpan.Zero, TimeSpan.FromMinutes(CLEANUP_MINUTES));
        }
        ...
        public TryOptionAsync<string> LoadPage(Option<LoadPageRequest> request)
        {
            ...
            var uri = request.Bind(r => r.Uri).Match(u => u, () => throw new Exception("Firefox dirver: Uri empty"));
            ...
            return async () =>
            {
                ...
                try
                {
                    ...
                    {
                        _logger.LogWarning("Document state is not ready. Attempt to parse source anyway");
                        //throw new Exception($"Failed to load page within {WAIT_FOR_PAGE_LOAD_IN_SECONDS}s - document not ready");
                    }

                    await WebDriverServiceFirefox.ExecuteUserActions(_logger,
                    container, userActions, correlationId).Match(r => r, () => throw new Exception("User Actions failed"), ex => throw new Exception("User Actions Failed", ex));
                ...
                {
                    _logger.LogError(ex, "Error connecting to Driver Management. Attempt to get source anyway");
            ...
            {
                return d => container.Driver.ExecuteScript("return document.readyState").Equals("complete");
            }
        }
        ...
        private TryOptionAsync<FirefoxContainer> CreateDriver(string uri)
        {
            return async () =>
            {
                return await GetFirefoxContainer(new Uri(uri));
            };
        }
